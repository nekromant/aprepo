...
- **Q**: Does the spec handle backend needing "-o arch=xxx" and multiple calls to a backend to get several xapk archives, one for each enabled arch in the config? → **A**: Yes. For store sources (`google_play`, `rustore`, `apkpure`), the download backend MUST be invoked once per package per configured architecture in `settings.architectures`. GitHub and WebDL sources MAY optionally configure per-architecture download targets (`arch_masks` for GitHub, `arch_files` for WebDL). If an architecture is unavailable or its per-architecture configuration is not specified, the system MUST log a warning and skip that architecture for the package.
- **Q**: What happens when a store backend does NOT support per-architecture downloads? → **A**: Store backends vary in capability. The system MUST first attempt per-architecture downloads for each configured architecture. If the backend returns an error indicating per-architecture downloads are unsupported (or produces a universal XAPK despite the arch option), the system MUST fall back to a single universal download for that source. The per-architecture capability result (supported or unsupported) is recorded in `state.yaml` per source. On subsequent runs, if a source is recorded as unsupported, the system goes directly to universal download. The `--force` flag bypasses the cached capability and re-tests per-architecture downloads. In both cases — per-architecture XAPKs or a single universal multi-arch XAPK — the process step produces the same set of output APKs per configured architecture.
- **Q**: What are the exact steps required to correctly repack an XAPK into a standalone installable APK, and why did the initial implementation produce `INSTALL_FAILED_MISSING_SPLIT`? → **A**: A naive merge that simply copies the base APK fails because Android still sees the `isSplitRequired="true"` manifest attribute and split metadata, but the actual split APKs are no longer present. The correct merge requires a full apktool decode/merge/rebuild cycle documented in FR-009c: (1) extract XAPK, (2) classify APKs by type (main/arch/dpi/locale), (3) decode all APKs with `apktool d -s`, (4) merge `lib/` from arch splits, (5) merge `res/` from DPI/locale splits, (6) merge `assets/assetpack/`, (7) delete bundle signing metadata (`BNDLTOOL.*`), (8) fix misnamed `.png` files that are actually JPEG, (9) update `AndroidManifest.xml` to remove split-required attributes and change stamp type to `STAMP_TYPE_STANDALONE_APK`, (10) rebuild with `apktool b`, (11) `zipalign -p -f 4`, (12) sign with `apksigner` if enabled.
- **Q**: During real-world testing, a per-architecture XAPK downloaded for `arm64-v8a` actually contained only `armeabi-v7a` splits and was repacked under the wrong architecture name. Should the system validate the actual architecture in a downloaded XAPK before repacking? → **A**: Yes. Before repacking a per-architecture XAPK, the system MUST validate that the downloaded file actually contains the requested architecture (or a valid fallback). If the XAPK's `manifest.json` shows no matching architecture split, the system MUST emit a warning and skip processing for that architecture. The absence of native code (no `lib/` directory) MUST NOT be treated as an error — apps without native code are valid for all architectures. This validation applies only to XAPK files; direct-download APK files MAY lack native code entirely and must not be skipped for architecture reasons. See FR-009d.
- **Q**: The system maintains an internal list of valid Android ABIs. Should the configuration file be validated to reject unknown architectures at startup? → **A**: Yes. At startup, the system MUST validate that every architecture listed in `settings.architectures` is a recognized Android ABI. The canonical valid architecture list is: `arm64-v8a`, `armeabi-v7a`, `armeabi`, `x86_64`, `x86`. If any invalid or misspelled architecture is present in the configuration, the system MUST print a clear fatal error listing the invalid values and exit with a non-zero status code before any downloads begin. See FR-018b.
...
- What happens when a download fails or produces a corrupt file? See FR-003a for full retry and fallback behavior.
...
- What happens when a per-architecture download is unavailable or fails? For store sources, if the backend returns no file for a requested architecture, the system logs a warning and skips that architecture. For GitHub/WebDL, if an architecture in `settings.architectures` has no matching `arch_masks`/`arch_files` entry, the system logs a warning and skips that architecture. Successfully downloaded architectures for the same package are preserved. See FR-003i.
- What happens when a store backend does not support per-architecture downloads? The system attempts per-arch downloads on first run. If the backend errors or returns a universal XAPK, the system falls back to a single universal download, logs a warning, and records the source as "unsupported" in `state.yaml`. On subsequent runs, that source goes directly to universal download. `--force` re-tests per-arch capability.
...
- **FR-003c**: The system MUST process downloads fully sequentially — one download request at a time — with a configurable per-source delay between individual download requests. A "download request" is defined as a single package for a single architecture (or a single universal package when per-architecture downloads are not configured).
...
- **FR-003e**: For GitHub releases configured with `throttle_policy: metadata`, the system MUST query the GitHub API for the latest release of the configured repository and use the release `tag_name` as the version identifier. The system MUST compare the latest release tag against the version stored in `state.yaml` for the package and only download the matching asset(s) if the tag has changed. For `arch_masks` configurations, the same release tag applies to all architectures.
- **FR-003f**: For store sources (`google_play`, `rustore`, `apkpure`), the system MUST first attempt to invoke the download backend once per package per configured architecture in `settings.architectures`. Each invocation MUST pass the target architecture to the backend (e.g., `-o arch=arm64-v8a`). The resulting files MUST be cached with an architecture suffix in their filenames (e.g., `com.example.app_arm64-v8a.xapk`). If the backend does not support per-architecture downloads (see FR-003j), the system MUST fall back to a single universal download for that source.
- **FR-003j**: The system MUST detect whether a store backend supports per-architecture downloads. On the first download attempt for a source, the system MUST attempt per-architecture downloads. If the backend returns a per-architecture-specific error or produces a universal XAPK instead of an architecture-specific one, the system MUST record the source as "unsupported" in `state.yaml` and fall back to a single universal download. On subsequent runs, if a source is recorded as "unsupported", the system MUST skip per-architecture attempts and download a single universal file directly. When `--force` is passed, the system MUST re-attempt per-architecture downloads even for sources recorded as "unsupported", re-evaluating the capability.
- **FR-003g**: For GitHub packages, the system MUST support an optional `arch_masks` field containing a map of architecture strings to file masks (e.g., `arm64-v8a: "*-arm64.apk"`). When `arch_masks` is present and non-empty, the system MUST download one asset per architecture listed in both `settings.architectures` and `arch_masks`, using the corresponding mask. If an architecture appears in `settings.architectures` but has no entry in `arch_masks`, the system MUST log a warning and skip that architecture for the package. When `arch_masks` is absent or empty, the system MUST fall back to the default `mask` for a single universal download.
- **FR-003h**: For WebDL packages, the system MUST support an optional `arch_files` field containing a map of architecture strings to objects with `filename` and `url` fields. When `arch_files` is present and non-empty, the system MUST download one file per architecture listed in both `settings.architectures` and `arch_files`, using the corresponding `filename` and `url`. If an architecture appears in `settings.architectures` but has no entry in `arch_files`, the system MUST log a warning and skip that architecture for the package. When `arch_files` is absent or empty, the system MUST fall back to the default `filename` and `url` for a single universal download.
- **FR-003i**: If a per-architecture download fails (network error, corrupt file, missing asset, or backend returns no file), the system MUST log a warning, preserve any successfully downloaded architectures for that package, and leave the failed architecture unchanged in the cache. The failed architecture MUST be retried on the next application run. The overall package download is considered partially successful if at least one architecture is downloaded successfully.
...
- **FR-004a**: For every package configured with a `metadata`-capable backend, the system MUST check remote version metadata **before** initiating the download, even when cached files already exist. If the cached version for a given architecture matches the remote version, the system MUST skip that architecture's download and print an informational message (e.g., `Cache hit: <package_id> — version <version> unchanged, skipping download`). If the cached version differs, the system MUST print an informational message (e.g., `New version available: <package_id> — remote <remote_version> vs cached <cached_version>, downloading...`) before downloading. If no cached version exists for an architecture, the system MUST print an informational message (e.g., `Fresh download: <package_id>`) before downloading. For store sources and GitHub, the version check is performed once per package and applies to all configured architectures. For WebDL with `arch_files`, the version check is performed per architecture. This behavior MUST be explicitly covered by integration tests for all metadata-capable backends: GitHub, WebDL, and apkeep-backed sources (APKPure, RuStore) where `throttle_policy: metadata` is supported.
...
- **FR-007**: The system MUST place all raw downloads and internal state into a designated cache directory and MUST manage retention so that only the current version plus a configurable number of previous versions per package are kept; older versions MUST be purged during a dedicated cleanup phase that runs after processing. The cache and output paths MUST be explicitly defined in the configuration file. If a configured directory does not exist at startup, the system MUST create it automatically and print a warning. The cache and output paths MUST be explicitly defined in the configuration file. If a configured directory does not exist at startup, the system MUST create it automatically and print a warning.
...
- **FR-009**: When `settings.repack_xapk` is `true`, the system MUST repack each cached XAPK bundle into standalone APKs. The system MUST inspect the XAPK manifest to determine which architecture splits are present. For universal XAPKs (containing splits for multiple architectures), the system MUST produce one APK per architecture that is both present in the XAPK and listed in `settings.architectures`. For per-architecture XAPKs (containing a single architecture split, typical of store-source downloads), the system MUST produce exactly one APK for that architecture. For each architecture being produced, the system MUST merge the base APK with the appropriate architecture split and all available density splits using the density fallback priority order. If an architecture listed in `settings.architectures` is missing from a universal XAPK (even after fallback), that architecture MUST be skipped and a warning MUST be logged. When `settings.repack_xapk` is `false`, the system MUST copy XAPK files to the output directory unchanged.
...
- **FR-009c**: When `settings.repack_xapk` is `true`, the XAPK-to-APK merge MUST follow this exact sequence for each target architecture:
  1. **Extract** the XAPK as a ZIP archive into a temporary directory.
  2. **Classify** all contained APKs by type: `main` (base APK matching `package_name.apk` or `base.apk`), `arch` (`config.<abi>.apk`), `dpi` (`config.<density>dpi.apk`), and `locale` (all other `config.*.apk` files).
  3. **Decode** every APK (base + all splits) with `apktool d -s <apk>` into separate decoded directories. The `-s` flag skips Smali disassembly, preserving only resources, manifest, and native libraries.
  4. **Merge architecture splits** into the decoded base directory: copy the entire `lib/` subtree from each architecture split APK into the base APK's `lib/` directory. If multiple architecture splits are merged (e.g., fallback from `arm64-v8a` to `armeabi-v7a`), copy them in fallback priority order so that the preferred ABI's libraries take precedence.
  5. **Merge DPI splits** into the decoded base directory: copy the `res/` subtree from each DPI split into the base APK's `res/` directory, skipping `values/public.xml` and skipping any `drawable*` files that already exist in the base APK. Merge the `doNotCompress` entries from each split's `apktool.yml` into the base `apktool.yml`.
  6. **Merge locale splits** into the decoded base directory: copy the `res/` subtree and `assets/assetpack/` subtree from each locale split into the base APK. Merge `doNotCompress` entries from locale splits' `apktool.yml`.
  7. **Delete bundle signing metadata** from the decoded base APK: remove `original/META-INF/BNDLTOOL.RSA`, `original/META-INF/BNDLTOOL.SF`, and `original/META-INF/MANIFEST.MF` if present.
  8. **Fix misnamed image files** in the decoded base APK's `res/` directory: scan all `.png` files; if a file's first three bytes are `FF D8 FF` (JPEG magic), rename it to `.jpg` and update any `apktool.yml` references.
  9. **Update AndroidManifest.xml** in the decoded base APK to remove split-apk metadata that would cause `INSTALL_FAILED_MISSING_SPLIT`:
     - Remove `android:isSplitRequired="true"` attribute.
     - Remove `android:requiredSplitTypes="base__abi,base__density"` attribute.
     - Remove `android:splitTypes=""` attribute.
     - Change `android:value="STAMP_TYPE_DISTRIBUTION_APK"` to `android:value="STAMP_TYPE_STANDALONE_APK"`.
     - Remove `<meta-data android:name="com.android.vending.splits.required" android:value="true"/>` element.
     - Remove `<meta-data android:name="com.android.vending.splits" android:resource="@xml/splits0"/>` element.
  10. **Rebuild** the standalone APK with `apktool b <base_apk_dir>`. The output is produced in `<base_apk_dir>/dist/<base_name>.apk`.
  11. **Zipalign** the rebuilt APK with `zipalign -p -f 4 <in> <out>`. The `-p` flag ensures page alignment required for Android runtime.
  12. **Sign** the aligned APK with `apksigner sign` if `settings.sign.enabled` is `true`.
- **FR-009d**: Before repacking a per-architecture XAPK (one that was downloaded for a specific target architecture), the system MUST validate that the XAPK actually contains the requested architecture. The validation MUST inspect the `manifest.json` `split_apks` entries and check whether any split's `abi` or filename matches the target architecture (or the fallback order defined in FR-009). If the downloaded XAPK does not contain the requested architecture and no fallback architecture is present either, the system MUST emit a warning naming the expected and actual architectures found, and MUST skip repacking that file for the specified architecture. The absence of native libraries (no `lib/` directory in any split) MUST NOT be treated as a mismatch; apps without native code are valid for all architectures. This validation is only required for XAPK files; direct-download APK files MAY omit native code entirely and MUST NOT be skipped for architecture reasons.
- **FR-019**: The downloaded file extension (`.apk` or `.xapk`) MUST be preserved in the cache directory. When `settings.repack_xapk` is `false`, files MUST be copied to the output directory with their original extension unchanged. When `settings.repack_xapk` is `true`, repacked output files MUST always use the `.apk` extension regardless of the original input extension.
...
- **FR-012b**: The system MUST deduplicate error and warning messages within a single invocation. If the same error or warning text would be printed more than once (e.g., a missing token reported for every package in a source), it MUST be printed exactly once.
...
- **FR-020**: When invoked without any arguments (no subcommand, no `--config`, no flags), the CLI MUST print the full help/usage information to stdout and exit with status code 0. This behavior MUST be covered by an integration test.
- **FR-021**: If a write failure occurs during download (e.g., disk full), the system MUST abort the current download, clean up the partial file, report the error, and preserve existing cache contents without corruption.
...
- **FR-018b**: The system MUST validate at startup that every architecture listed in `settings.architectures` is a recognized Android ABI. The canonical valid architecture list is: `arm64-v8a`, `armeabi-v7a`, `armeabi`, `x86_64`, `x86`. If any invalid or misspelled architecture is present in the configuration, the system MUST print a clear fatal error listing the invalid values and exit with a non-zero status code before any downloads begin.
...
- `architectures` (list of strings, default `[arm64-v8a]`): Target ABIs for downloads, XAPK merging, and output APK naming. Store backends are invoked once per architecture. GitHub and WebDL per-architecture configurations are filtered by this list. Supported values: `arm64-v8a`, `armeabi-v7a`, `armeabi`, `x86_64`, `x86`.
...
- `repack_xapk` (boolean, default `false`): When `true`, the system repacks each cached XAPK bundle into standalone APKs. For universal XAPKs, one APK is produced per architecture found in the XAPK and listed in `settings.architectures`. For per-architecture XAPKs, exactly one APK is produced for that architecture. Signing is applied if configured. When `false`, XAPK files are copied to the output directory unchanged.
...
- `github`:
  ...
  - `packages` (list of objects):
    ...
    - `mask` (string, required): Default file pattern for matching release assets when `arch_masks` is not used (e.g., `*.apk`). If multiple assets in a single release match the mask, the system selects the first match sorted alphabetically and logs a warning listing all matched asset names.
    - `arch_masks` (object, optional): Map of architecture strings to file masks for per-architecture downloads (e.g., `arm64-v8a: "*-arm64.apk"`). When present, the system downloads one asset per architecture listed in both `settings.architectures` and `arch_masks`. Architectures in `settings.architectures` without a matching `arch_masks` entry are skipped with a warning.
- `webdl`:
  - `throttle_policy`, `throttle_interval`, `delay_between_requests`: Same semantics as other sources.
  - `packages` (list of objects):
    - `filename` (string, required): Default local filename for storing the downloaded file when `arch_files` is not used. The file extension SHOULD be `.apk`.
    - `url` (string, required): Default direct download URL when `arch_files` is not used. This URL is used as the stable identifier for tracking the package in `state.yaml` (via a hash of the URL). After download, the actual Android package name is extracted from the APK's manifest for uniqueness validation and output naming.
    - `arch_files` (object, optional): Map of architecture strings to objects with `filename` and `url` fields for per-architecture downloads (e.g., `arm64-v8a: { filename: "app-arm64.apk", url: "https://example.com/app-arm64.apk" }`). When present, the system downloads one file per architecture listed in both `settings.architectures` and `arch_files`. Architectures in `settings.architectures` without a matching `arch_files` entry are skipped with a warning.

**Validation rules**:
- Duplicate package names across store sources (`google_play`, `rustore`, `apkpure`): see FR-018. GitHub and WebDL package uniqueness: see FR-018a.
...
- **SC-001**: 100% of packages with available version metadata are re-downloaded only when the version changes; no redundant downloads occur between version changes.
- **SC-002**: Packages configured with `throttle_policy: dumb` are re-downloaded no more frequently than the configured throttle interval.
- **SC-003**: After processing, 100% of files in the output directory are valid artifacts: standalone APK files for direct downloads and repacked XAPKs; when `settings.repack_xapk` is `false`, original XAPK files are also permitted. No ZIP-wrapped artifacts or partial files remain.
- **SC-004**: The system recovers gracefully from interruption: after a forced stop during download or processing, the next invocation produces the same result as if the previous run had completed successfully.
- **SC-005**: Adding support for a new download source requires changes only to the backend integration layer, with zero modifications to the configuration schema or CLI interface.
- **SC-006**: Integration tests in `tests/integrational` verify that: (a) invalid configurations produce a non-zero exit code; (b) each download source fixture produces a well-formed ZIP in the cache directory; (c) the process phase produces output APKs that pass `aapt2 dump badging` validation; (d) both direct-token and `$VARIABLE` syntax for credentials are exercised and function correctly; (e) a synthetic XAPK fixture with base + split APKs validates both `repack_xapk: true` (repacked APKs in output) and `repack_xapk: false` (XAPK copied unchanged) paths; (f) metadata-capable backends (GitHub, WebDL, and apkeep-backed sources where supported) are tested for cache-hit behavior: a second download run with unchanged metadata MUST skip the download and print a cache-hit message, while a changed metadata scenario MUST trigger a fresh download and print a new-version message.
...
- Store backends (`apkeep`-backed sources) may or may not support per-architecture download options (e.g., `-o arch=xxx`). The system attempts per-architecture downloads first and falls back to a single universal download when unsupported. Capability is detected at runtime and cached in `state.yaml` per source.
