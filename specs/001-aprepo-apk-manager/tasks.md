...
- [ ] T017a [US2] Implement XAPK Architecture Validation in `src/process/xapk.rs` — per FR-009d, before repacking a per-architecture XAPK, inspect `manifest.json` `split_apks` to verify the downloaded file actually contains the requested architecture (or valid fallback). Emit a warning and skip if mismatch. Absence of native code (`lib/`) is NOT an error.
