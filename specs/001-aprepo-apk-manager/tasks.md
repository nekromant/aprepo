...
- [X] T017a [US2] Implement XAPK Architecture Validation in `src/process/xapk.rs` — per FR-009d, `check_arch_mismatch()` inspects `manifest.json` `split_apks` to verify the downloaded file actually contains the requested architecture exactly (no fallback for per-arch files). Emits a warning naming expected vs actual architectures and skips if mismatch. Absence of native code (no arch splits at all) is explicitly NOT treated as an error. Covered by 6 unit tests: exact match, filename match, wrong arch, no native code, universal with match, universal without target.
